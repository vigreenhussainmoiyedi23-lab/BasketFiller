...
Router.get("/stateEnum", async (req, res) => {
  ...
  res
    .status(200)
    .json({ enumValues, message: "Succesfully fetched the enum values" });
});
...
Router.get("/user", UserCanAcces, async (req, res) => {
  ...
  return res
    .status(200)
    .json({ messgae: "all orders of the user", orders: user.Orders });
});
...
Router.post(
  ...
  async (req, res) => {
    ...
    if (CartItems.length === 0) {
      return res
        .status(400)
        .json({ message: "User Must Have Something In the cart" });
    }
